...
namespace {
std::string get_or_declare_string_parameter(const std::shared_ptr<rclcpp::Node> &node, const std::string &name,
...
ROS2Visualizer::~ROS2Visualizer() { stop_image_publisher_thread(); }

void ROS2Visualizer::start_image_publisher_thread() {
  if (!_app->get_params().use_multi_threading_pubs) {
    return;
  }

  bool expected = false;
  if (!image_thread_running.compare_exchange_strong(expected, true)) {
    return;
  }

  image_publisher_thread = std::thread([this] {
    rclcpp::Rate loop_rate(20);
    while (rclcpp::ok() && image_thread_running.load()) {
      publish_images();
      loop_rate.sleep();
    }
  });
...
void ROS2Visualizer::stop_image_publisher_thread() {
...
void ROS2Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  std::string topic_imu = get_or_declare_string_parameter(_node, "topic_imu", "/imu0");
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    // Read in the topics
    std::string cam_topic0 = get_or_declare_string_parameter(_node, "topic_camera0", "/cam0/image_raw");
    std::string cam_topic1 = get_or_declare_string_parameter(_node, "topic_camera1", "/cam1/image_raw");
    ...
    auto latest0 = std::make_shared<sensor_msgs::msg::Image::UniquePtr>();
    auto latest1 = std::make_shared<sensor_msgs::msg::Image::UniquePtr>();
    auto sync_mutex = std::make_shared<std::mutex>();
    auto try_emit = [this, latest0, latest1, sync_mutex]() {
      sensor_msgs::msg::Image::UniquePtr msg0;
      sensor_msgs::msg::Image::UniquePtr msg1;
      {
        std::lock_guard<std::mutex> lock(*sync_mutex);
        if (!*latest0 || !*latest1) {
          return;
        }
        const double t0 = (*latest0)->header.stamp.sec + (*latest0)->header.stamp.nanosec * 1e-9;
        const double t1 = (*latest1)->header.stamp.sec + (*latest1)->header.stamp.nanosec * 1e-9;
        if (std::fabs(t0 - t1) > 0.010) {
          if (t0 < t1) {
            latest0->reset();
          } else {
            latest1->reset();
        ...
        msg0 = std::move(*latest0);
        msg1 = std::move(*latest1);
      }
      callback_stereo_unique(std::move(msg0), std::move(msg1), 0, 1);
    };
    auto sub0 = _node->create_subscription<sensor_msgs::msg::Image>(
        cam_topic0, rclcpp::SensorDataQoS(),
        [latest0, try_emit](sensor_msgs::msg::Image::UniquePtr msg) {
          *latest0 = std::move(msg);
          try_emit();
        },
        sub_options);
    auto sub1 = _node->create_subscription<sensor_msgs::msg::Image>(
        cam_topic1, rclcpp::SensorDataQoS(),
        [latest1, try_emit](sensor_msgs::msg::Image::UniquePtr msg) {
          *latest1 = std::move(msg);
          try_emit();
        },
        sub_options);
    subs_cam.push_back(sub0);
    subs_cam.push_back(sub1);
    ...
    for (int i = 0; i < _app->get_params().state_options.num_cameras; i++) {
      ...
      std::string cam_topic =
          get_or_declare_string_parameter(_node, "topic_camera" + std::to_string(i), "/cam" + std::to_string(i) + "/image_raw");
      ...
      auto sub = _node->create_subscription<sensor_msgs::msg::Image>(
          ...
          [this, i](sensor_msgs::msg::Image::UniquePtr msg0) { callback_monocular_unique(std::move(msg0), i); }, sub_options);
...
void ROS2Visualizer::callback_inertial(const sensor_msgs::msg::Imu::SharedPtr msg) {
  static bool printed_first_imu = false;
  if (!printed_first_imu) {
    PRINT_INFO("received first IMU measurement\n");
    printed_first_imu = true;
  }
...
void ROS2Visualizer::callback_monocular_unique(sensor_msgs::msg::Image::UniquePtr msg0, int cam_id0) {
  ...
  // Get the image while sharing the ROS message storage when the encoding already matches.
  auto tracked = std::shared_ptr<void const>(std::move(msg0));
  const auto &image_msg = *std::static_pointer_cast<const sensor_msgs::msg::Image>(tracked);
  cv_bridge::CvImageConstPtr cv_ptr;
  try {
    cv_ptr = cv_bridge::toCvShare(image_msg, tracked, sensor_msgs::image_encodings::MONO8);
  } catch (cv_bridge::Exception &e) {
    PRINT_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
...
void ROS2Visualizer::callback_stereo(const sensor_msgs::msg::Image::ConstSharedPtr msg0, const sensor_msgs::msg::Image::ConstSharedPtr msg1,
  ...
  static bool printed_first_stereo = false;
  if (!printed_first_stereo) {
    PRINT_INFO("received first stereo image pair\n");
    printed_first_stereo = true;
  }
...
void ROS2Visualizer::callback_stereo_unique(sensor_msgs::msg::Image::UniquePtr msg0, sensor_msgs::msg::Image::UniquePtr msg1, int cam_id0,
                                            int cam_id1) {
  static bool printed_first_stereo = false;
  if (!printed_first_stereo) {
    PRINT_INFO("received first stereo image pair\n");
  ...
  auto tracked0 = std::shared_ptr<void const>(std::move(msg0));
  auto tracked1 = std::shared_ptr<void const>(std::move(msg1));
  const auto &image_msg0 = *std::static_pointer_cast<const sensor_msgs::msg::Image>(tracked0);
  const auto &image_msg1 = *std::static_pointer_cast<const sensor_msgs::msg::Image>(tracked1);

  cv_bridge::CvImageConstPtr cv_ptr0;
  try {
    cv_ptr0 = cv_bridge::toCvShare(image_msg0, tracked0, sensor_msgs::image_encodings::MONO8);
  } catch (cv_bridge::Exception &e) {
    PRINT_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
  ...
  cv_bridge::CvImageConstPtr cv_ptr1;
  try {
    cv_ptr1 = cv_bridge::toCvShare(image_msg1, tracked1, sensor_msgs::image_encodings::MONO8);
  } catch (cv_bridge::Exception &e) {
    PRINT_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
